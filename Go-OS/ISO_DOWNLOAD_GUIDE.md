...
**Note:** The GhostOS build script uses `debootstrap` to download and create Debian 12 directly from official repositories. **You do NOT need to download the ISO to build GhostOS.**
...
**Note:** The GhostOS build script (`ghostos-build.sh`) downloads Debian 12 automatically via `debootstrap` and does not require the ISO file.
